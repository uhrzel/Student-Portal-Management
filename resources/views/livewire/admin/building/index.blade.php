<div>
    <div class="py-12">
        ...
        <div class="max-w-7xl mx-auto space-y-4 sm:px-6 lg:px-8">
            <nav class="flex" aria-label="Breadcrumb">
                <ol class="inline-flex items-center space-x-1 md:space-x-2 rtl:space-x-reverse">
                    <li class="inline-flex items-center">
                        <a href="{{ route('admin.buildings') }}" class="inline-flex items-center text-sm font-medium text-gray-700 hover:text-blue-600">
                            Building
                        </a>
                    </li>
                    <li>
            ...
            <div class="flex flex-row justify-between items-center">
                <h1 class="uppercase text-2xl font-bold text-yellow-700 text-start">Buildings</h1>
                <a wire:navigate href="{{ route('admin.buildings.create') }}">
                ...
                <div class="overflow-x-auto">
                    <table class="w-full text-sm text-left text-gray-500">
                        <thead class="text-xs text-gray-700 uppercase bg-gray-50">
                            <tr>
                                <th scope="col" class="px-4 py-3">Building Name</th>
                        ...
                        <tbody>
                            ...
                            <tr wire:key="{{ $building->id }}" class="border-b">
                                <th scope="row" class="px-4 py-3 font-medium text-gray-900 whitespace-nowrap">{{ $building->name }}</th>
                                <td class="px-4 py-3">{{ $building->status }}</td>
                                    ...
                                    <div x-data="{ open: false }">
                                        ...
                                        <div x-show="open" @click.away="open = false" class="absolute z-10 w-44 right-10 bg-white rounded divide-y divide-gray-100 shadow">
                                            ...
                                            <ul class="py-1 text-sm text-gray-700">
                                                <li>
                                                    <a wire:navigate href="{{ route('admin.buildings.edit', $building->id) }}" class="block py-2 px-4 hover:bg-gray-100">Edit</a>
                                                </li>
                                            </ul>
                                            <div class="py-1">
                                                <button @click="$dispatch('open-modal', { id: {{ $building->id }} })" class="block w-full text-left py-2 px-4 text-sm text-red-700 hover:bg-gray-100">Delete</button>
                                            </div>
                                            @endif
                                            @if($building->deleted_at !== null)
                                            <div class="py-1">
                                                <button @click="$dispatch('restore-building', { id: {{ $building->id }} })" class="block w-full text-left py-2 px-4 text-sm text-gray-700 hover:bg-gray-100">Restore</button>
                                            </div>
                                            <div class="py-1">
                                                <button @click="$dispatch('force-delete-building', { id: {{ $building->id }} })" class="block w-full text-left py-2 px-4 text-sm text-red-700 hover:bg-gray-100">Permanently Delete</button>
                                            </div>
                                            @endif
                                        </div>
                                    </div>
                            ...
                            @empty
                            <tr>
                                <td colspan="5" class="px-4 py-3 text-center">No buildings found.</td>
                            </tr>
                            @endforelse
                        </tbody>
                    </table>
    ...
    <!-- Modal Delete -->
    <div x-data="{ showModal: false, buildingId: null }"
        @open-modal.window="showModal = true; buildingId = $event.detail.id"
        x-show="showModal"
            ...
            <div class="relative bg-white rounded-lg shadow-xl max-w-lg w-full">
                <div class="p-6">
                    ...
                    <div class="mt-4 flex space-x-3">
                        ...
                        <button @click="$wire.deleteBuilding(buildingId); showModal = false" type="button" class="inline-flex justify-center px-4 py-2 text-sm font-medium text-white bg-red-600 border border-transparent rounded-md hover:bg-red-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-red-500">
                            Delete
                        </button>
                    </div>
                </div>
    ...
    <!-- Modal Restore -->
    <div x-data="{ showRestoreModal: false, buildingId: null }"
        @restore-building.window="showRestoreModal = true; buildingId = $event.detail.id"
            ...
            <div class="relative bg-white rounded-lg shadow-xl max-w-lg w-full">
                <div class="p-6">
                    ...
                    <div class="mt-4 flex space-x-3">
                        ...
                        <button @click="$wire.restoreBuilding(buildingId); showRestoreModal = false" type="button" class="inline-flex justify-center px-4 py-2 text-sm font-medium text-white bg-green-600 border border-transparent rounded-md hover:bg-green-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-green-500">
                            Restore
                        </button>
                    </div>
                </div>
    ...
    <!-- Modal Force Delete -->
    <div x-data="{ showForceDeleteModal: false, buildingId: null }"
        @force-delete-building.window="showForceDeleteModal = true; buildingId = $event.detail.id"
            ...
            <div class="relative bg-white rounded-lg shadow-xl max-w-lg w-full">
                <div class="p-6">
                    ...
                    <div class="mt-4 flex space-x-3">
                        ...
                        <button @click="$wire.forceDeleteBuilding(buildingId); showForceDeleteModal = false" type="button" class="inline-flex justify-center px-4 py-2 text-sm font-medium text-white bg-red-600 border border-transparent rounded-md hover:bg-red-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-red-500">
                            Delete Permanently
                        </button>
                    </div>
                </div>
