<div>
    <div class="py-12">
        ...
        <div class="max-w-7xl mx-auto space-y-4 sm:px-6 lg:px-8">
            ...
            <div class="flex flex-row justify-between items-center">
                ...
                <a wire:navigate href="{{ route('admin.rooms.create') }}">
                    ...
                    <table class="w-full text-sm text-left text-gray-500">
                        ...
                        <tbody>
                            ...
                            <tr wire:key="{{ $room->id }}" class="border-b">
                                <th scope="row" class="px-4 py-3 font-medium text-gray-900 whitespace-nowrap">{{ $room->name ?? 'N/A' }}</th>
                                <td class="px-4 py-3">{{ $room->building->name ?? 'N/A' }}</td>
                                    ...
                                    <div x-data="{ open: false }">
                                        ...
                                        <div x-show="open" @click.away="open = false" class="absolute z-10 w-44 right-10 bg-white rounded divide-y divide-gray-100 shadow">
                                            ...
                                            <ul class="py-1 text-sm text-gray-700">
                                                <li>
                                                    <a wire:navigate href="{{ route('admin.rooms.edit', $room->id) }}" class="block py-2 px-4 hover:bg-gray-100">Edit</a>
                                                </li>
                                            </ul>
                                            <div class="py-1">
                                                <button @click="$dispatch('open-modal', { id: {{ $room->id }} })" class="block w-full text-left py-2 px-4 text-sm text-red-700 hover:bg-gray-100">Delete</button>
                                            </div>
                                            @endif
                                            @if($room->deleted_at !== null)
                                            <div class="py-1">
                                                <button @click="$dispatch('restore-room', { id: {{ $room->id }} })" class="block w-full text-left py-2 px-4 text-sm text-gray-700 hover:bg-gray-100">Restore</button>
                                            </div>
                                            <div class="py-1">
                                                <button @click="$dispatch('force-delete-room', { id: {{ $room->id }} })" class="block w-full text-left py-2 px-4 text-sm text-red-700 hover:bg-gray-100">Permanently Delete</button>
                                            </div>
                                            @endif
                                        </div>
                                    </div>
                            ...
                            @empty
                            <tr>
                                <td colspan="5" class="px-4 py-3 text-center">No rooms found.</td>
                            </tr>
                            @endforelse
                        </tbody>
                    </table>
    ...
    <!-- Modal Delete -->
    <div x-data="{ showModal: false, roomId: null }"
        @open-modal.window="showModal = true; roomId = $event.detail.id"
        x-show="showModal"
        ...
        <div class="flex items-center justify-center min-h-screen px-4">
            ...
            <div class="relative bg-white rounded-lg shadow-xl max-w-lg w-full">
                <div class="p-6">
                    ...
                    <p class="mt-2 text-sm text-gray-500">Are you sure you want to delete this room? This action cannot be undone.</p>
                    ...
                    <div class="mt-4 flex space-x-3">
                        ...
                        <button @click="$wire.deleteRoom(roomId); showModal = false" type="button" class="inline-flex justify-center px-4 py-2 text-sm font-medium text-white bg-red-600 border border-transparent rounded-md hover:bg-red-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-red-500">
                            Delete
                        </button>
                    </div>
                </div>
    ...
    <!-- Modal Restore -->
    <div x-data="{ showRestoreModal: false, roomId: null }"
        @restore-room.window="showRestoreModal = true; roomId = $event.detail.id"
            ...
            <div class="relative bg-white rounded-lg shadow-xl max-w-lg w-full">
                <div class="p-6">
                    ...
                    <div class="mt-4 flex space-x-3">
                        ...
                        <button @click="$wire.restoreRoom(roomId); showRestoreModal = false" type="button" class="inline-flex justify-center px-4 py-2 text-sm font-medium text-white bg-green-600 border border-transparent rounded-md hover:bg-green-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-green-500">
                            Restore
                        </button>
                    </div>
                </div>
    ...
    <!-- Modal Force Delete -->
    <div x-data="{ showForceDeleteModal: false, roomId: null }"
        @force-delete-room.window="showForceDeleteModal = true; roomId = $event.detail.id"
            ...
            <div class="relative bg-white rounded-lg shadow-xl max-w-lg w-full">
                <div class="p-6">
                    ...
                    <div class="mt-4 flex space-x-3">
                        ...
                        <button @click="$wire.forceDeleteRoom(roomId); showForceDeleteModal = false" type="button" class="inline-flex justify-center px-4 py-2 text-sm font-medium text-white bg-red-600 border border-transparent rounded-md hover:bg-red-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-red-500">
                            Delete Permanently
                        </button>
                    </div>
                </div>
